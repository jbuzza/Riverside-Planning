...
It should be noted that the council website does not restrict the number of comments an individual can make. In some cases, multiple comments have been submitted. One reason for this is that the length of each comment is resticted.
...
The imported comments data was consolidated with comment type used as a factor. The postcode for each comment was extracted from the address as a distinct variable.
...
kable(missing, caption =  "Comments listed without postcodes")
...
Since multiple comments can be made from the same address we can see where this has occured and determine whether these should be combined. It should be noted that these are not duplicate comments and typically occur becuase of addiitonal points or due to the character restriction imposed when submitting comments.
...
map2disp <- ggmap(map, extent= "device", legend="bottomright") + geom_point(data = df_grouped, aes(x = Longitude, y = Latitude, color = Type,size=n),alpha=1)+scale_size(guide = 'none')+ggtitle("Map of all comments")
...
Objections are fairly evenly spread around an area bounded by the A316 and the river. 

```{r fig.width = 8, fig.height=8}
map <- get_map(geo_location,source="google",maptype="roadmap", zoom=13,scale=2)

map2disp <- ggmap(map, extent= "device", legend="bottomright") + geom_point(data = df_grouped, aes(x = Longitude, y = Latitude, color = Type,size=n),alpha=1)+scale_size(guide = 'none')+ggtitle("Map of  comments from Twickenham area")

print(map2disp)
```

By increasing zoom level further to 15 it can be seen that there are distinct concentrations of supporting comments. Firstly, from Eel Pie Island which overlooks the site and secondly, from an area around Haggard Road / Victoria Road which would appear to coincide with the address given by the Riverside Action Group.

```{r fig.width = 8, fig.height=8}
map <- get_map(geo_location,source="google",maptype="roadmap", zoom=15,scale=2)

map2disp <- ggmap(map, extent= "device", legend="bottomright") + geom_point(data = df_grouped, aes(x = Longitude, y = Latitude, color = Type,size=n),alpha=1)+scale_size(guide = 'none')+ggtitle("Map of  comments nearest to development")
